...
import { DeletePlanServices } from '../../services/authServices';
...
export const PlanList = () => {
  ...
  const { deletePlan, delIsLoading} = DeletePlanServices();
  ...
  const handleEditPlan = (planId : number)=>{
    ...
    navigate('/edit-plan', { 
      state: { planId: planId}  // 可选：传递参数 
    });
  }

  const handleDelPlan = async (planId : number)=>{
      const id = planId
      ...
      if (result.success === true ) {
        console.log("删除计划成功");
        // 删除成功后，刷新列表数据
        setData(data.filter(item => item.planId !== planId));
      } else {
        console.log("删除计划失败");
      }
      delIsLoading;
  ...
  const handletoTask = (planId: number) =>{
    const id = planId
    navigate('/tasks', { state: { planId: id } });
  }
...
const columns: ColumnsType<PlanDataType> = [
  ...
  },
  {
    ...
    <button 
    onClick={()=>{handleEditPlan(record.planId)}}
    aria-label="编辑计划">
      编辑
      </button>
    ...
    <button 
    onClick={()=>{handleDelPlan(record.planId)}}
    aria-label="删除计划">
      删除
      </button>
    ...
    <button 
    onClick={()=>{handletoTask(record.planId)}}
    aria-label="创建任务">
      详情
      </button>
