...
export const RegisterPage: React.FC =()=>{
    ...
    const handleSumbit  = async (e:React.FormEvent) =>{
        ...
        try{
            ...
            const result = await register(registerformState);
            if (result){
                navigate('/login',{state:{message:'Registration successful'}})
            }else{
                setErrors({ ...errors, username: 'Registration failed. Please check your input.' });
                navigate('/register/', { state: { message: 'Registration failed. Please try again.' } });
            }
    ...
    <form onSubmit={handleSumbit} className="register-page">
        ...
        <div className="form-group">
            <label htmlFor="permissions_level">Permissions Level</label>
            <input type="number" id="permissions_level"
             placeholder="Enter your permissions level"
              value={registerformState.permissions_level}
               onChange={(e)=>setRegisterformState({
                ...registerformState,permissions_level:Number(e.target.value)})}
                disabled={isLoading}
            />
            {errors.permissions_level && <div className="error">{errors.permissions_level}</div>}
        </div>
        <button type="submit"
